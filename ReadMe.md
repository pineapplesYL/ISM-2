...
└── 成员代码/          # 存放各成员开发的代码
    ...
    └── CuiJingyan/     # 成员崔靖炎个人代码目录
        └── QRCodeGenerator.py  # 动态QR码解析与生成工具
```
...
**功能介绍：**  
本工具是一款基于Flask框架构建的Web应用，集成了QR码解析、动态内容调整与生成功能，主要特性包括：

1. **QR码解析与内容提取**  
   - 支持用户上传PNG、JPG、JPEG、GIF格式的图片，自动检测并解析其中的QR码内容。
   - 采用OpenCV的微信二维码检测算法，提升复杂场景下的识别准确率。

2. **动态时间参数调整**  
   - 自动检测QR码内容中的时间戳字段（如`createTime`），并对其进行修改，适应时区或时间校准需求。

3. **自定义QR码生成**  
   - 允许用户输入任意文本或调整后的内容，实时生成高容错率的QR码图片。
   - 支持自定义复杂度（1-40）、格子像素大小、边框宽度等参数，满足不同场景需求。

4. **安全文件上传与预览**  
   - 限制上传文件大小（≤2MB）及类型，通过文件名安全校验防止路径攻击。
   - 生成QR码以Base64编码直接嵌入网页，无需本地存储，保障数据隐私。

5. **可视化交互界面**  
   - 提供网页端表单上传、解析结果展示、调整后内容对比及生成QR码的一站式操作体验。

**技术亮点：**  
- 结合Flask实现轻量级Web服务，前端模板与静态资源分离，便于扩展。
- 采用`qrcode`库生成QR码，结合OpenCV实现高效解码。
- 基于Base64的图片内存处理，避免敏感数据落盘，提升安全性。
